class ZoomAuthController < ApplicationController
  before_action :authenticate_user!
  before_action :set_app_credentials
  before_action :require_career_officer, only: [ :auth, :disconnect ]
  ...
  def auth
    ...
    Rails.logger.info("Using client ID: #{@client_id[0..5]}...")
  ...
  def callback
    ...
    if params[:state] != session[:zoom_oauth_state]
      ...
      redirect_to career_officer_meetings_path
      return
    end
  ...
  def disconnect
    if current_user.zoom_credential&.destroy
      Rails.logger.info("Zoom credentials removed for user #{current_user.id}")
      flash[:notice] = "Disconnected from Zoom successfully."
    else
      Rails.logger.warn("No Zoom credentials found to remove for user #{current_user.id}")
      flash[:alert] = "Zoom was not connected or an error occurred."
    end
    redirect_to career_officer_meetings_path
  end
